...
class RoutinePerformanceApp:
    ...
    def insert_routine_performance_data(self) -> None:
        ...
        self.document.template_variables["m28_percent_seen_in_30"] = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'MOV28 Pct Routine Referrals Seen in 30d')))

        variance = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'Var MOV91 Pct Routine Referrals Seen in 30d')))
        self.document.template_variables["var_m28_percent_seen_in_30"] = variance
        self.document.template_variables["dir_m28_percent_seen_in_30"] = (
            wt.get_overall_measure(wt.last_month, 'Dir Var MOV91 Pct Routine Referrals Seen in 30d'))
        if variance < 0:
            self.document.template_variables["dir_m28_percent_seen_in_30_style"] = "card-data-direction-down-sm"
        else:
            self.document.template_variables["dir_m28_percent_seen_in_30_style"] = "card-data-direction-up-sm"

        self.document.template_variables["m91_percent_seen_in_30"] = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'MOV91 Pct Routine Referrals Seen in 30d')))

        variance = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'Var MOV182 Pct Routine Referrals Seen in 30d')))
        self.document.template_variables["var_m91_percent_seen_in_30"] = variance
        self.document.template_variables["dir_m91_percent_seen_in_30"] = (
            wt.get_overall_measure(wt.last_month, 'Dir Var MOV182 Pct Routine Referrals Seen in 30d'))
        if variance < 0:
            self.document.template_variables["dir_m91_percent_seen_in_30_style"] = "card-data-direction-down-sm"
        else:
            self.document.template_variables["dir_m91_percent_seen_in_30_style"] = "card-data-direction-up-sm"

        self.document.template_variables["m182_percent_seen_in_30"] = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'MOV182 Pct Routine Referrals Seen in 30d')))

        variance = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'Var MOV364 Pct Routine Referrals Seen in 30d')))
        self.document.template_variables["var_m182_percent_seen_in_30"] = variance
        self.document.template_variables["dir_m182_percent_seen_in_30"] = (
            wt.get_overall_measure(wt.last_month, 'Dir Var MOV364 Pct Routine Referrals Seen in 30d'))
        if variance < 0:
            self.document.template_variables["dir_m182_percent_seen_in_30_style"] = "card-data-direction-down-sm"
        else:
            self.document.template_variables["dir_m182_percent_seen_in_30_style"] = "card-data-direction-up-sm"

        self.document.template_variables["m364_percent_seen_in_30"] = (
            v.half_up_int(wt.get_overall_rate_measure(wt.last_month, 'MOV364 Pct Routine Referrals Seen in 30d')))
    ...
    # END insert_routine_performance_data
# END CLASS RoutinePerformanceApp
